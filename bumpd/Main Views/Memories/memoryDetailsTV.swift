...
class memoryDetailsTV: UITableViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        let vc = segue.destination as? memoryMap
        vc?.memory = self.memory
        
        let vc2 = segue.destination as? memoryPhoto
        vc2?.memory = self.memory
    ...
    func setupBump() {
        ...
        let ref1 = databaseRef.child("Feed/\(memory.id)/Memory")
        let ref2 = databaseRef.child("Users/\(memory.author)")
        let ref3 = databaseRef.child("Users/\(memory.recipient)")
        ...
        if memory.author == uid && memory.recipient != uid {
            ...
            ref3.observe(.value) { (snapshot) in
                ...
                self.detailsLabel.text = "You bumpd into \(name) at \(self.memory.details) on \(self.memory.date)"
                
                self.recipientImg.loadImageUsingCacheWithUrlString(urlString: img)
                
            }
            
        } else if memory.recipient == uid && memory.author != uid {
    ...
    func checkComments() {
        ...
        databaseRef.child("Feed/\(memory.id)/Notes").observe(.value) { (snapshot) in
