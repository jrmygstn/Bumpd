...
class bumpDetailsView: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        let vc = segue.destination as? bumpDetailsTV
        vc?.bumps = self.bumps
    ...
    func addToMemories() {
        ...
        let ref = databaseRef.child("Feed/\(bumps.id)/Notes")
    ...
    func notifyRecipient() {
        ...
        let auth = bumps.author
        let text = self.messageField.text!
        let recip = bumps.recipient
        let uid = Auth.auth().currentUser?.uid
            ...
            storedImage.putData(uploadData, metadata: nil, completion: { (metadata, error) in
                ...
                storedImage.downloadURL(completion: { (url, error) in
                    ...
                    if let urlText = url?.absoluteString{
                        self.databaseRef.child("Feed/\(self.bumps.id)/Memory").updateChildValues(["img" : urlText], withCompletionBlock: { (error, ref) in
                            if error != nil{
                                print(error!)
                                return
                            }
